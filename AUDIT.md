...
| Area | Posture |
|------|---------|
| `@fabric/core` | Git pin `FabricLabs/fabric#feature/rsi` (lockfile SHA `ff7c05c52c38c39e552c6f4a4bc62442425bdd0e`; Node **24.15.0**) |
...
10. **PR #69 auth boundary** — Hub self-sign is **opt-in** (`allowHubSelfSign === true`) and **loopback-only**; LiveRelay omits unrecognized Bearer when `issueBearer` is absent; device-link origins are canonicalized for replay keys (`offerReplayKey` + create path); peer host parsing is IPv6-safe and rejects ports outside **1..65535**. Site-login / LiveRelay share `clientMayPollDesktopSession` for pending polls. Off-loopback signed-session redeem and device-link DELETE require `pollSecret` via `X-Fabric-Poll-Secret` only (`?pollSecret=` is ignored). Expired-session GET `/sessions/:delegationToken` requires a matching `Authorization: Bearer` (path is not a credential). `sessionId` is now **bound into the attested message** (`fabric:device-link:2` prefix, server-generated nonce, prepare/commit split; v1 still parses so clients can roll forward). Remaining: bump the remaining consumers (Passport / SCL) onto the v2 builders, and device-link GET `linked` stays Origin-gated for the QR responder. Own-host DNS uses `dns.promises.lookup` (cached).
11. **Follow-up (ops):** keep `package.json` on `FabricLabs/fabric#feature/rsi` during RSI development, but **re-pin releases** to the lockfile SHA (`ff7c05c5` as of 2026-08-25). `npm run report:install` removes `package-lock.json` then `npm i --allow-git=all` — bump the tip with `npm install FabricLabs/fabric#feature/rsi --allow-git=all` (or pin a SHA) when core moves.

### PR #69 review triage (`feature/rsi`)

Latest automation security review still flags Origin as insufficient — signed redeem now uses `pollSecret`. Core pin refreshed to `ff7c05c5` ([core #186](https://github.com/FabricLabs/fabric/pull/186): `Tree#verifyInclusion` no longer lets the prover choose the commitment, NOISE `onverify` will not restore a freed pointer, `frameId` hex validation, `Store#flush` during deferred open; earlier [core #185](https://github.com/FabricLabs/fabric/pull/185): IdentityCrossSign compressed 66-hex ids, `_normPubkey`, candidate-retry bound, NOISE teardown, Filesystem publish retain cut, file-count fold). Stale PR threads still describe Hub self-sign default-on and `wss:` origin bypass — both are **fixed** in this tree. JSONCall unauthorized-hash / watch-only `signWithKey`, peer port `1..65535`, 402 digest coercion, ARC type-only reject, federation `accept` boolean, and device-link `offerReplayKey` origin canonicalize threads are also **fixed** (threads remain unresolved on GitHub). CodeRabbit’s RFC6902 round-trip suggestion on `tests/messageBodyJsonBridge.test.js` is a **false positive**: the assertion matches the second constructed message (`{ a: 1 }`), not the first `rateSats` helper object. CodeRabbit 402 markup-from-`costBasisSats` is **wontfix** (privacy). `--wallet -p` is not a path.
...
| `GET /sessions/:id` signed redeem via forgeable Origin | Fixed — `pollSecret` header only (`X-Fabric-Poll-Secret`; no query); pending GET stays Origin-gated |
| LiveRelay Bearer redeem same poll gate | Fixed — same `getSession` check |
| Device-link DELETE cancel via forgeable Origin | Fixed — `pollSecret` off-loopback |
| Device-link GET attestation disclosure | Open — responder has QR `sessionId` only; pending/accepted/linked GET stay Origin-gated |
| Device-link `sessionId` bind in attest message | Fixed here — `fabric:device-link:2` binds `sessionId` + server nonce (v1 still parses). Consumer bump (Passport / SCL) outstanding |
...
| Pin `@fabric/core` to SHA for releases | Ops — keep `#feature/rsi` in RSI; lockfile records `ff7c05c5` |
