...
- **Site-login / device-link Origin gates (PR #69 High, partial)** — Hub self-sign is **opt-in** (`allowHubSelfSign === true`; Hub desktop enables it) and **loopback-only** (no remote Origin-gated self-sign). Site-login and LiveRelay share `clientMayPollDesktopSession` (`functions/fabricSiteLoginVerify.js`). Device-link GET/create uses that gate **plus** thin-client Origins on an **allowlisted** hub (`clientMayAccessDeviceLink` in `fabricDeviceLinkHttp.js`: Capacitor / loopback WebView / `chrome-extension:` / `moz-extension:`). `GET /sessions/:id` redeem, LiveRelay Bearer redeem, and device-link GET still treat matching `Origin`/`Referer`/`Sec-Fetch-Site` (or a thin-client Origin on an allowlisted hub) as authorization. Those headers are forgeable on non-browser clients; QR/`fabric://` URLs expose `sessionId`. Needs a possession proof (e.g. one-time poll secret or signed browser challenge) before treating this as browser-grade auth on shared hosts. Keep Hub origin allowlists fail-closed; never treat `X-Forwarded-*` as proof of loopback.
...
- ~~**`@fabric/core` pin hygiene**~~ — lockfile tip `3745041e3f50f484b188ab8e6cb03f515fcccca2` (refreshed via `FabricLabs/fabric#feature/rsi`). Coordinate Hub / apps on the same SHA for releases; `report:install` wipes the lockfile then `npm i --allow-git=all` (nested git SHA prepare). Bump the tip with `npm install FabricLabs/fabric#feature/rsi --allow-git=all` when core moves.
...
- **Peer host self-check DNS** — IPv6 bracketed addresses parse correctly; ports must be decimal **1..65535** (`parseFabricPeerPort`); DNS cache keys include `ownHosts`. Remaining: migrate `dns.lookupSync` → async `dns.promises.lookup` (Node 24 deprecation).
...
- **Site-login delegation token** — fail-closed when `expected` supplies only one of `sessionId` / `origin`; expired-session GET requires opaque Bearer registry key + `timingSafeEqual` bind to path `sessionId`; LiveRelay omits Bearer when `issueBearer` is absent (no orphan random token). Remaining: Hub dual-key legacy cleanup if any path still stores under `sessionId`.
