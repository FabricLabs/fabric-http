...
## Hub allowlist
`FABRIC_HUB_ALLOWLIST` — default network hubs + loopback; extras via `FABRIC_HUB_ALLOWLIST` (exact origins preferred; optional HTTPS-only operator suffixes like `*.hub.example.com`). Shared platforms (`*.vercel.app`) and public multi-part suffixes (`*.co.uk`) are rejected — use an exact preview origin instead. Unknown origins must not receive signed login/link completions.
...
- ~~**RFC6902 sidechain JSON bridge**~~ — multi-op sequences are preserved in `patchesCanonical` (`functions/messageBodyJsonBridge.js`; `tests/messageBodyJsonBridge.test.js`). Catalog still seeds from the first `/registry` op.
