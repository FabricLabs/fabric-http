...
- ~~**`@fabric/core` pin hygiene**~~ — lockfile tip `2e4d2ac75e8e37260791cfd5503480cdc0915317` (refreshed via `FabricLabs/fabric#feature/rsi`). Coordinate Hub / apps on the same SHA for releases; plain `report:install` keeps the lockfile so bump with `npm install FabricLabs/fabric#feature/rsi --allow-git=all` when core moves.
