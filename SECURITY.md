...
- ~~**`@fabric/core` pin hygiene**~~ — lockfile tip `3e5dafff33af306c097656b69ddb2d74bd101fb0` (refreshed via `feature/rsi` + `npm run report:install`). Coordinate Hub / apps on the same SHA for releases.
...
- **Peer host self-check DNS** — IPv6 bracketed addresses parse correctly; DNS cache keys include `ownHosts`. Remaining: migrate `dns.lookupSync` → async `dns.promises.lookup` (Node 24 deprecation).
