...
- ~~**`@fabric/core` pin hygiene**~~ — lockfile tip `3c96383430f9f233a2f2be0850c0f2eb4c8366fb` (refreshed via `FabricLabs/fabric#feature/rsi`). Coordinate Hub / apps on the same SHA for releases; plain `report:install` keeps the lockfile so bump with `npm install FabricLabs/fabric#feature/rsi --allow-git=all` when core moves.
