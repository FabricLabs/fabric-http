...
- ~~**`@fabric/core` pin hygiene**~~ — lockfile tip `51ad619c9c6ed937f586db8ca69262e2f205e2d3` (refreshed via `FabricLabs/fabric#feature/rsi`). Coordinate Hub / apps on the same SHA for releases; plain `report:install` keeps the lockfile so bump with `npm install FabricLabs/fabric#feature/rsi --allow-git=all` when core moves.
