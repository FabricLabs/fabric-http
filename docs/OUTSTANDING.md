...
- `@fabric/core` lockfile **`88f766c28`**: [#186](https://github.com/FabricLabs/fabric/pull/186) NOISE handshake-bus + gossip catalog + entropy harden (includes [#185](https://github.com/FabricLabs/fabric/pull/185) Filesystem retain cut / MuSig2 / BIP-21 / collection cwd-containment / `fabricIdentityAccountPath`). Re-pin after Wave 1 core honesty lands on a new tip.
...
## PRs
[#69](https://github.com/FabricLabs/fabric-http/pull/69) — Cursor Highs on Origin-gated **signed** redeem / device-link cancel are addressed with `pollSecret` (not extra Origin checks). Device-link GET pending/accepted/linked stays Origin-gated for the QR responder. Remaining coordinated work: client header send + bind `sessionId` into attest messages. CodeRabbit 402 markup-from-`costBasisSats` is **wontfix** (privacy). Codacy reports **0** new issues (`tests/**` excluded). Pin `@fabric/core` via lockfile (`#feature/rsi`), currently **`88f766c28`** ([core #186](https://github.com/FabricLabs/fabric/pull/186)). `puppeteer@25.7.0` (extract-zip) is deferred. Docstring 80% gate is deferred. GitHub/CodeRabbit file-count vs FabricLabs/master is the vendor `dist` + JSDoc HTML deletes — restore those trees rather than deleting them on this PR. RFC6902 multi-op `messageBodyJsonBridge` remains deferred.
