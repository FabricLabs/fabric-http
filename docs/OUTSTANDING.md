...
**Last reviewed:** 2026-08-16 ([Hub crash scan](https://relay.goon.vc/downstream.agents.md) 03:58Z; http GitHub `1b162e35` includes DELETE `/device-links/:sessionId` cancel; core lockfile `f1b5e147`). [#69](https://github.com/FabricLabs/fabric-http/pull/69) CodeRabbit: `cancelDeviceLinkSession` fetch rejection is `ok: false` (staged). Highs on Origin-gated redeem / device-link `sessionId` bind remain **blockers** (no unilateral possession proof; DELETE uses the same Origin gate as GET). Already in tree: Hub self-sign loopback, `wss:` origin map, HTTPS allowlist, `offerReplayKey` canonicalize, CHANGELOG `sessionId` is not possession, `security@fabric.pub`, SLIP-0044 row-count, peer port `1..65535`, WS close wait, commit Transaction assertion.
...
## Next slices
- [ ] Production Hub still pins http `61bb801` ([crash scan](https://relay.goon.vc/downstream.agents.md)): sibling-NIC self-filter in `collectOwnFabricHosts` (`1b162e35`) is **not live**, so Hub↔RSI Fabric ESTAB stays missing. Deploy this pin with Hub `992ea478`.
...
- `@fabric/core` lockfile **`f1b5e147`**: MuSig2 `autoAccept` default off, BIP-21 `req-*`, collection cwd-containment, Codacy `Number('…')` uint32/purpose literals; plus `fabricIdentityAccountPath` / AMP wire name over inventory JSON `type: 98` (locked in `tests/pr69.review.coverage.js` + `tests/identityCrossSign.reexport.test.js`).
...
## PRs
[#69](https://github.com/FabricLabs/fabric-http/pull/69) — Cursor review restates **2 High + 2 Medium** on Origin-gated redeem / device-link `sessionId` bind (still open; DELETE Origin gate is the same issue, not a new protocol). CodeRabbit cancel fetch-failure `ok: false` is staged. CodeRabbit 402 markup-from-`costBasisSats` is **wontfix** (privacy). Federation invite policy/`accept`, ARC type-only, 402 blob ids, `wss:` origin, JSONCall hash, and `--wallet` are **already in tree**. Pin `@fabric/core` via lockfile (`#feature/rsi` + `npm run report:install`), currently **`f1b5e147`** ([core #185](https://github.com/FabricLabs/fabric/pull/185)). GitHub http HEAD is **`1b162e35`** (device-link DELETE cancel is on that tip). `puppeteer@25.7.0` (extract-zip) is deferred.
