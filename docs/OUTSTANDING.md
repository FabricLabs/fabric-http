...
**Last reviewed:** 2026-08-14 (http `5161e76` + this slice, core lockfile `1fc616492`).
...
- IdentityCrossSign / identity Schnorr / verify re-export `@fabric/core` (commit/push core before this repo’s CI, or `npm link @fabric/core`). `fabricSiteLoginVerify` re-exports `resolveFabricSigningIdentity`.
- `canonicalizeFabricPeerDial` rewrites `hub.fabric.pub` / `relay.goon.vc` `:7778` → `:7777` (and dedicated NIC IPs `65.21.231.166` / `65.21.231.149`) and drops self-dials (playnet RSI error-log storm). `pubkey@host:port` splits to the same host as `host:port`.
