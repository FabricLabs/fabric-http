...
**Last reviewed:** 2026-08-14 (http `17abf49` + this slice, core lockfile `4d7351ee3`).
...
## Next slices
- [ ] Re-export `identityCrossSign` / `identityCrossSignVerify` / `fabricIdentitySchnorr` from core once those leaves are on the `@fabric/core` pin (local http WIP exists; do not enable — core `4d7351ee3` does not ship them). Keep `package.json` exports pointing at local copies until then, or CI `MODULE_NOT_FOUND`.
