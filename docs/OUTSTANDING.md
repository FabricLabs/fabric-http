...
**Last reviewed:** 2026-08-16 ([Hub crash scan](https://relay.goon.vc/downstream.agents.md) 03:58Z; http GitHub `ff781c5` plus local DELETE cancel; core lockfile `f1b5e147`). [#69](https://github.com/FabricLabs/fabric-http/pull/69) comment rescan: Highs on Origin-gated redeem / device-link `sessionId` bind remain **blockers** (no unilateral possession proof). Already in tree: Hub self-sign loopback, `wss:` origin map, HTTPS allowlist, `offerReplayKey` canonicalize, CHANGELOG `sessionId` is not possession, `security@fabric.pub`, SLIP-0044 row-count, peer port `1..65535`, WS close wait, commit Transaction assertion.
...
## Next slices
- [ ] Production Hub still pins http `61bb801` ([crash scan](https://relay.goon.vc/downstream.agents.md)): sibling-NIC self-filter in `collectOwnFabricHosts` (`ff781c5`) is **not live**, so Hub↔RSI Fabric ESTAB stays missing. Deploy this pin with Hub `250a178`.
